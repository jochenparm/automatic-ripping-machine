...
from fadr.config import config_utils
from fadr.config.config_utils import fadr_yaml_test_bool
from fadr.models.alembic_version import AlembicVersion
...
from fadr.ui.metadata import (call_omdb_api, get_tmdb_poster, tmdb_find,
                              tmdb_search)
from fadr.ui.settings import DriveUtils

# Path definitions
path_migrations = "fadr/migrations"
...
def check_db_version(install_path, db_file):
    ...
    if not os.path.isfile(db_file):
        app.logger.info("No database found.  Initializing fadr.db...")
        ...
        if head_revision == db_version:
            ...
            if head_revision == db_version:
                ...
                app.logger.error(f"Database is still out of date. "
                                 f"Head is {head_revision} and database is {db_version}.  Exiting fadr.")


def fadr_alembic_get():
    """
    ...
    install_path = cfg.fadr_config['INSTALLPATH']

    # Get the fadr alembic current head revision
    mig_dir = os.path.join(install_path, path_migrations)
    config = Config()
    config.set_main_option("script_location", mig_dir)
    script = ScriptDirectory.from_config(config)
...
def fadr_db_check():
    ...
    Check if db exists and is up-to-date.
    """
    db_file = cfg.fadr_config['DBFILE']
    db_exists = False
    db_current = False
    head_revision = None
    db_revision = None

    head_revision = fadr_alembic_get()

    # Check if the db file exists
    if os.path.isfile(db_file):
        db_exists = True
        # Get the database alembic version
        db_revision = fadr_db_get()
...
def fadr_db_cfg():
    ...
    """
    db_update = fadr_db_check()
    if not db_update["db_exists"]:
        app.logger.debug("No fadrui cfg setup")
        check_db_version(cfg.fadr_config['INSTALLPATH'], cfg.fadr_config['DBFILE'])
        setup_database()

    # if the database has been updated
    # UISettings could be incorrect, return None
    try:
        fadrui_cfg = UISettings.query.get(1)
        app.jinja_env.globals.update(fadrui_cfg=fadrui_cfg)
    except Exception as e:
        app.logger.debug(f"fadr_cfg request error {e}")
        fadrui_cfg = None

    app.logger.debug(fadrui_cfg)
...
def fadr_db_migrate():
    ...
    db_revision = fadr_db_get()
    app.logger.info(f"FADR head: {head_revision} database: {db_revision.version_num}")
    if head_revision == db_revision.version_num:
        ...
        app.logger.error(
            ...
            f"is {db_revision.version_num}.  Exiting fadr.")


def fadr_db_initialise():
    """
    Initialise the FADR DB, ensure system values and disk drives are loaded
    """
...
def get_info(directory):
    ...
    for i in os.listdir(directory):
        if os.path.isfile(os.path.join(directory, i)):
            ...
            create_time = strftime(cfg.fadr_config['DATE_FORMAT'], localtime(file_stats.st_ctime))
            access_time = strftime(cfg.fadr_config['DATE_FORMAT'], localtime(file_stats.st_atime))
...
def generate_fadr_cat(full_path):
    """
    Read from log file and only output FADR: logs
    :param full_path: full path to job logfile
    :return: None
    """
...
def metadata_selector(func, query="", year="", imdb_id=""):
    ...
    if cfg.fadr_config['METADATA_PROVIDER'].lower() == "tmdb":
    ...
    elif cfg.fadr_config['METADATA_PROVIDER'].lower() == "omdb":
...
def fix_permissions(j_id):
    ...
    FADR can sometimes have issues with changing the file owner, we can use the fact FADRui is run
    ...
    if not job.path:
        ...
        validate_logfile(job.logfile, "true", Path(os.path.join(str(cfg.fadr_config['LOGPATH']), job.logfile)))
        # Find the correct path to use for fixing perms
        directory_to_traverse = find_folder_in_log(os.path.join(cfg.fadr_config['LOGPATH'], job.logfile),
    ...
    try:
        ...
        # If set media owner in fadr.yaml was true set them as users
...
def send_to_remote_db(job_id):
    """
    Send a local db job to the fadr remote crc64 database
    :param job_id: Job id
    :return: dict/json to return to user
    """
    job = Job.query.get(job_id)
    return_dict = {}
    api_key = cfg.fadr_config['FADR_API_KEY']
...
def find_folder_in_log(job_log, default_directory):
    ...
    Opens the job logfile and searches for fadr.ripper failing to set permissions\n
...
def trigger_restart():
    ...
    This only works if FADRui is running as a service & in debug mode

    notes: This has been removed, breaks and causes errors when run as 'fadr' user
    """
    ...
    now = datetime.now()
    fadr_main = os.path.join(os.path.dirname(os.path.abspath(__file__)), "routes.py")
    set_file_last_modified(fadr_main, now)


def build_fadr_cfg(form_data, comments):
    """
    Main function for saving new updated fadr.yaml\n
    :param form_data: post data
    :param comments: comments file loaded as dict
    :return: full new fadr.yaml as a String
    """
...
def build_apprise_cfg(form_data):
    ...
    for key, value in form_data.items():
        ...
        try:
            ...
            # Test if value is Boolean
            apprise_cfg += fadr_yaml_test_bool(key, value)
...
def validate_logfile(logfile, mode, my_file):
    ...
    :raise FileNotFoundError: if logfile cant be found in fadr log folder
    """
...
def get_git_revision_hash() -> str:
    ...
    try:
        git_hash = subprocess.check_output(['git', 'rev-parse', 'HEAD'],
                                           cwd=cfg.fadr_config['INSTALLPATH']).decode('ascii').strip()
...
def git_check_updates(current_hash) -> bool:
    """
    Check the FADR commit hash against the remote (GitHub) commit hash
    :param
        current_hash: str - string of current FADR commit hash
    :return:
        fadr_current: Bool - True for no update (or exceptions), False for update possible
    """
    ...
    fadr_current = True      # set True, any exceptions will return a true value
    ...
    try:
        ...
        fadr_current = latest_commit.startswith(current_hash)

        app.logger.debug(f"Remote hash: {latest_commit}")
        app.logger.debug(f"Local hash: {current_hash}")
        app.logger.debug(f"FADR current: {fadr_current}")
...
def git_check_version():
    """
    Check the current FADR version locally against the remote (GitHub) version.

    This function compares the installed FADR version with the latest version available
    ...
    try:
        ...
        app.logger.debug(f"Error - FADR Local Version file not found: {e}")
    except OSError as e:
        app.logger.debug(f"Error - FADR Local Version file error: {e}")
    ...
    try:
        ...
        app.logger.debug(f"Error - FADR Remote Version error: {e}")
...
def authenticated_state() -> bool:
    ...
    """
    authenticated = False
    if cfg.fadr_config['DISABLE_LOGIN']:
        authenticated = True
    else:
