"""
FADR route blueprint for database pages
Covers
- database [GET]
- dbupdate [POST]
- import_movies [JSON]
...
from fadr.ui.metadata import get_omdb_poster
...
def view_database():
    ...
    return render_template('databaseview.html',
                           ...
                           date_format=cfg.fadr_config['DATE_FORMAT'])


@route_database.route('/dbupdate', methods=['POST'])
def update_database():
    """
    Update the FADR database when changes are made or the fadr db file is missing
    """
    form = DBUpdate(request.form)
    if request.method == 'POST' and form.validate():
        if form.dbfix.data == "migrate":
            app.logger.debug("User requested - Database migration")
            ui_utils.fadr_db_migrate()
            flash("FADR database migration successful!", "success")
        elif form.dbfix.data == "new":
            app.logger.debug("User requested - New database")
            ui_utils.check_db_version(cfg.fadr_config['INSTALLPATH'], cfg.fadr_config['DBFILE'])
            flash("FADR database setup successful!", "success")
        else:
            # No method defined
        ...
        # Update the fadr UI config from DB post update
        ui_utils.fadr_db_cfg()
...
def import_movies():
    """
    Function for finding all movies not currently tracked by FADR in the COMPLETED_PATH
    ...
    :return: Outputs json - contains a dict/json of movies added and a notfound list
             that doesn't match FADR identified folder format.
    .. note:: This should eventually be moved to /json page load times are too long
    """
    my_path = cfg.fadr_config['COMPLETED_PATH']
    ...
    for movie in movie_dirs:
        ...
        # if we can match the standard fadr output format "Movie (year)"
        if matched:
