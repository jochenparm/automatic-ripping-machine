...
from fadr.models.config import Config
...
from fadr.ui.forms import ChangeParamsForm
from fadr.ui.settings import DriveUtils as drive_utils  # noqa E402
from fadr.ui.utils import (authenticated_state, database_updater,
                           job_id_validator)
...
def get_x_jobs(job_status):
    ...
    for j in jobs:
        job_results[i] = {}
        job_log = os.path.join(cfg.fadr_config['LOGPATH'], str(j.logfile))
    ...
    return {"success": success,
            "mode": job_status,
            "results": job_results,
            "fadr_name": cfg.fadr_config['FADR_NAME'],
...
def process_audio_logfile(logfile, job, job_results):
    ...
    """
    # \((track[^[]+)(?!track)
    line = read_all_log_lines(os.path.join(cfg.fadr_config["LOGPATH"], logfile))
...
def search(search_query):
    """ Queries FADRui db for the movie/show matching the query"""
    ...
    try:
        ...
        if mode == 'delete' and job_id is not None:
            ...
            if job_id == 'all':
                ...
                #  if os.path.isfile(cfg.fadr_config['DBFILE']):  # noqa: S125
                #    # Make a backup of the database file
                #    cmd = f"cp {cfg.fadr_config['DBFILE']} {cfg.fadr_config['DBFILE'])}.bak"
...
def change_job_params(config_id):
    ...
    if form.validate():
        ...
        cfg.fadr_config["MINLENGTH"] = config.MINLENGTH = format(form.MINLENGTH.data)
        cfg.fadr_config["MAXLENGTH"] = config.MAXLENGTH = format(form.MAXLENGTH.data)
        cfg.fadr_config["RIPMETHOD"] = config.RIPMETHOD = format(form.RIPMETHOD.data)
        # must be 1 for True 0 for False
        cfg.fadr_config["MAINFEATURE"] = config.MAINFEATURE = 1 if format(form.MAINFEATURE.data).lower() == "true" else 0
...
def get_notify_timeout(notify_timeout):
    ...
    fadrui_cfg = UISettings.query.first()

    if fadrui_cfg:
        return_json['notify_timeout'] = fadrui_cfg.notify_refresh
    else:
...
def restart_ui():
    app.logger.debug("FADR ui shutdown....")
    shutdown_code = subprocess.check_output(
        "pkill python3",
        shell=True
    ).decode("utf-8")
    # Nothing should work past here as the ui will die after running code above
    app.logger.debug(f"FADR ui shutdown ran into a problem... exit code: {shutdown_code}")
