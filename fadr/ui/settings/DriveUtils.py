...
def drives_search():
    ...
    if len(udev_drives) > 0:
        app.logger.info(f"System disk scan, found {len(udev_drives)} drives for FADR")
        for disk in udev_drives:
            app.logger.debug(f"disk: {disk}")
    else:
        app.logger.info("System disk scan, no drives attached to FADR server")
...
def drives_update():
    ...
    if new_count > 0:
        app.logger.info(f"Added {new_count} drives for FADR.")
    else:
