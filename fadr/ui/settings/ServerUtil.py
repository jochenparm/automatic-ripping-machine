...
class ServerUtil():
    ...
    def get_update(self):
        ...
        self.storage_transcode_free, self.storage_transcode_percent = \
            self.get_disk_space(cfg.fadr_config['TRANSCODE_PATH'])
        self.storage_completed_free, self.storage_completed_percent = \
            self.get_disk_space(cfg.fadr_config['COMPLETED_PATH'])
    ...
    def get_disk_space(self, filepath):
        ...
        try:
            ...
            disk_space = 0
            disk_percent = 0
            app.logger.debug("FADR folders not found")
            flash("There was a problem accessing the FADR folder: "
                  f"'{filepath}'. Please make sure you have setup FADR", "danger")
