"""
FADR route blueprint for log pages
Covers
- logs [GET]
- listlogs [GET]
- logreader [GET]
...
def listlogs(path):
    ...
    Gives the user links to tail/fadr/Full/download
    """
    base_path = cfg.fadr_config['LOGPATH']
    ...
    return render_template('logfiles.html', files=files, date_format=cfg.fadr_config['DATE_FORMAT'])
...
def logreader():
    ...
    """
    log_path = cfg.fadr_config['LOGPATH']
    ...
    # Only FADR logs
    if mode == "fadrcat":
        generate = ui_utils.generate_fadr_cat(full_path)
