...
from fadr.ui import app, constants, db
from fadr.ui.forms import DBUpdate
from fadr.ui.settings.ServerUtil import ServerUtil
from fadr.ui.settings.settings import check_hw_transcode_support
...
def home():
    ...
    db_update = ui_utils.fadr_db_check()
    ...
    if not db_update["db_current"] or not db_update["db_exists"]:
        ...
        app.logger.debug(f"Error with FADR DB: [{db_update['db_current']}]-[{db_update['db_exists']}]")
    ...
    # Page titles
    session["fadr_name"] = cfg.fadr_config['FADR_NAME']
    ...
    return render_template("index.html",
                           ...
                           jobs=jobs,
                           children=cfg.fadr_config['FADR_CHILDREN'],
                           server=server, serverutil=serverutil,
                           fadr_path=fadr_path, media_path=media_path, stats=stats)
...
def setup():
    ...
    This function will do various checks to make sure everything can be setup for FADR
    ...
    #     return redirect("/")
    # dir0 = Path(PurePath(cfg.fadr_config['DBFILE']).parent)
    # dir1 = Path(cfg.fadr_config['RAW_PATH'])
    # dir2 = Path(cfg.fadr_config['TRANSCODE_PATH'])
    # dir3 = Path(cfg.fadr_config['COMPLETED_PATH'])
    # dir4 = Path(cfg.fadr_config['LOGPATH'])
    # fadr_directories = [dir0, dir1, dir2, dir3, dir4]

    # try:
    #     for fadr_dir in fadr_directories:
    #         if not Path.exists(fadr_dir):
    #             os.makedirs(fadr_dir)
    #             flash(f"{fadr_dir} was created successfully.", "success")
    ...
    # else:
    #     flash("Successfully created all of the FADR directories", "success")
    #     app.logger.debug("Successfully created all of the FADR directories")
