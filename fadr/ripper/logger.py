...
def setup_logging(job):
    ...
    if job.label == "" or job.label is None:
        ...
        log_full = os.path.join(cfg.fadr_config['LOGPATH'], logfile)
    else:
        valid_label = job.label.replace("/", "_")
        log_file_name = f"{valid_label}.log"
        new_log_file = f"{valid_label}_{job.stage}.log"
        temp_log_full = os.path.join(cfg.fadr_config['LOGPATH'], log_file_name)
        log_file = new_log_file if os.path.isfile(temp_log_full) else log_file_name
        log_full = os.path.join(cfg.fadr_config['LOGPATH'], log_file)
        job.logfile = log_file
    # Remove any root loggers
    clean_loggers()
    # Debug formatting
    if cfg.fadr_config['LOGLEVEL'] == "DEBUG":
        logging.basicConfig(filename=log_full,
                            format='[%(asctime)s] %(levelname)s FADR: %(module)s.%(funcName)s %(message)s',
                            datefmt=cfg.fadr_config['DATE_FORMAT'], level=cfg.fadr_config['LOGLEVEL'])
    else:
        logging.basicConfig(filename=log_full, format='[%(asctime)s] %(levelname)s FADR: %(message)s',
                            datefmt=cfg.fadr_config['DATE_FORMAT'], level=cfg.fadr_config['LOGLEVEL'])
...
def clean_up_logs(logpath, loglife):
    ...
    for log_dir in logs_folders:
        ...
        # Loop through each file in current folder and remove files older than set in fadr.yaml
...
def create_logger(app_name, log_level=logging.DEBUG, stdout=True, syslog=False, file=False):
    ...
    formatter = logging.Formatter('[%(asctime)s] %(levelname)s FADR: %(module)s.%(funcName)s %(message)s')

    if file:
        # create file logger handler
        file_handler = logging.FileHandler('/home/fadr/logs/fadr.log')
