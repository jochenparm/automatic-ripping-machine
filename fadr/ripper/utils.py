...
from fadr.models.notifications import Notifications
from fadr.models.system_drives import SystemDrives
from fadr.models.track import Track
from fadr.models.user import User
from fadr.ripper import apprise_bulk
from fadr.ui import db  # needs to be imported before models
...
def notify(job, title: str, body: str):
    ...
    if cfg.fadr_config["FADR_NAME"] != "":
        title = f"[{cfg.fadr_config['FADR_NAME']}] - {title}"

    # append Job ID if configured
    if cfg.fadr_config["NOTIFY_JOBID"] and job is not None:
    ...
    bash_notify(cfg.fadr_config, title, body)

    # Sent to remote sites
    # Create an Apprise instance
    apobj = apprise.Apprise()
    if cfg.fadr_config["PB_KEY"] != "":
        apobj.add('pbul://' + str(cfg.fadr_config["PB_KEY"]))
    if cfg.fadr_config["IFTTT_KEY"] != "":
        apobj.add('ifttt://' + str(cfg.fadr_config["IFTTT_KEY"]) + "@" + str(cfg.fadr_config["IFTTT_EVENT"]))
    if cfg.fadr_config["PO_USER_KEY"] != "":
        apobj.add('pover://' + str(cfg.fadr_config["PO_USER_KEY"]) + "@" + str(cfg.fadr_config["PO_APP_KEY"]))
    if cfg.fadr_config["JSON_URL"] != "":
        apobj.add(str(cfg.fadr_config["JSON_URL"]).replace("http://", "json://").replace("https://", "jsons://"))
    try:
    ...
    if cfg.fadr_config["APPRISE"] != "":
        try:
            apprise_bulk.apprise_notify(cfg.fadr_config["APPRISE"], title, body)
            logging.debug(f"apprise-config: {cfg.fadr_config['APPRISE']}")
        except Exception as error:  # noqa: E722
...
def notify_entry(job):
    ...
    if job.disctype in ["dvd", "bluray"]:
        if cfg.fadr_config["UI_BASE_URL"] == "":
            display_address = (f"http://{check_ip()}:{job.config.WEBSERVER_PORT}")
        else:
            display_address = str(cfg.fadr_config["UI_BASE_URL"])
...
def sleep_check_process(process_str, transcode_limit):
    ...
    :param str process_str: The process string from fadr.yaml
...
def move_movie_poster(final_directory, hb_out_path):
    """move movie poster\n
    ---------\n
    DEPRECIATED - FADR already builds the final path so moving is no longer needed"""
...
def scan_emby():
    ...
    if cfg.fadr_config["EMBY_REFRESH"]:
        logging.info("Sending Emby library scan request")
        url = f"http://{cfg.fadr_config['EMBY_SERVER']}:{cfg.fadr_config['EMBY_PORT']}/Library/Refresh?api_key={cfg.fadr_config['EMBY_API_KEY']}"  # noqa: E501
        try:
...
def delete_raw_files(dir_list):
    """
    Delete the raw folders from fadr after job has finished
    :param list dir_list: Python list containing strings of the folders to be deleted

    """
    if cfg.fadr_config["DELRAWFILES"]:
...
def get_cdrom_status(devpath):
    ...
    try:
        ...
        # Sometimes FADR will log errors opening hard drives. this check should stop it
...
def rip_music(job, logfile):
    ...
    abcfile = cfg.fadr_config["ABCDE_CONFIG_FILE"]
    if job.disctype == "music":
        logging.info("Disc identified as music")
        # If user has set a cfg.fadr_config file with FADR use it
...
def rip_data(job):
    ...
    if (make_dir(raw_path)) is False:
        ...
        if (make_dir(raw_path)) is False:
            logging.info(f"Could not create data directory: {raw_path}  Exiting FADR. ")
    ...
    # Added from pull 366
    cmd = f'dd if="{job.devpath}" of="{incomplete_filename}" {cfg.fadr_config["DATA_RIP_PARAMETERS"]} 2>> ' \
...
def set_permissions(directory_to_traverse):
    ...
    """
    if not cfg.fadr_config['SET_MEDIA_PERMISSIONS']:
        return False
    try:
        corrected_chmod_value = int(str(cfg.fadr_config["CHMOD_VALUE"]), 8)
        logging.info(f"Setting permissions to: {cfg.fadr_config['CHMOD_VALUE']} on: {directory_to_traverse}")
        ...
        for dirpath, l_directories, l_files in os.walk(directory_to_traverse):
            for cur_dir in l_directories:
                logging.debug(f"Setting path: {cur_dir} to permissions value: {cfg.fadr_config['CHMOD_VALUE']}")
                os.chmod(os.path.join(dirpath, cur_dir), corrected_chmod_value)

            for cur_file in l_files:
                logging.debug(f"Setting file: {cur_file} to permissions value: {cfg.fadr_config['CHMOD_VALUE']}")
...
def try_add_default_user():
    """
    Added to fix missmatch from the fadrui and fadrripper\n
    This will try to add a default user for the fadrui
    with the details\n
    Username: admin\n
    Password: password\n
    :return: None
    ...
    try:
        ...
        perm_file = Path(PurePath(cfg.fadr_config['INSTALLPATH'], "installed"))
...
def fadr_setup(fadr_log):
    """
    Setup fadr - Create all the directories we need for fadr to run
    ...
    """
    fadr_directories = [
        cfg.fadr_config['RAW_PATH'],
        cfg.fadr_config['TRANSCODE_PATH'],
        cfg.fadr_config['COMPLETED_PATH'],
        cfg.fadr_config['LOGPATH'],
    ]
    try:
        # Check db file is writeable
        if not os.access(cfg.fadr_config['DBFILE'], os.W_OK):
            fadr_log.error(f"Cant write to database file! Permission ERROR: {cfg.fadr_config['DBFILE']} - FADR Will Fail!")
            raise OSError
        # Check directories for read/write permission -> create if they don't exist
        for folder in fadr_directories:
            if not os.access(folder, os.R_OK):
                # don't raise as we may be able to create
                fadr_log.error(f"Cant read from folder, Permission ERROR: {folder} - FADR Will Fail!")
            if not os.access(folder, os.W_OK):
                fadr_log.error(f"Cant write to folder, Permission ERROR: {folder} - FADR Will Fail!")
                raise OSError
            if make_dir(folder):
                fadr_log.error(f"Cant create folder: {folder} - FADR Will Fail!")
                raise OSError
    except OSError as error:
        fadr_log.error(
            f"A fatal error has occurred. "
            "Cant find/create the folders set in fadr.yaml "
            f"- Error:{error} - FADR Will Fail!"
        )
...
def check_ip():
    ...
    """
    if cfg.fadr_config['WEBSERVER_IP'] != 'x.x.x.x':
        return cfg.fadr_config['WEBSERVER_IP']
...
def save_disc_poster(final_directory, job):
    ...
    :return: None
    """
    if job.disctype == "dvd" and cfg.fadr_config["RIP_POSTER"]:
...
def check_for_dupe_folder(have_dupes, hb_out_path, job):
    ...
    :param have_dupes: is this title in the local fadr database
    ...
    if (make_dir(hb_out_path)) is False:
        ...
        logging.debug(f"Value of ALLOW_DUPLICATES: {cfg.fadr_config['ALLOW_DUPLICATES']}")
        logging.debug(f"Value of have_dupes: {have_dupes}")
        if cfg.fadr_config["ALLOW_DUPLICATES"] or not have_dupes:
            ...
            if not (make_dir(hb_out_path)):
                ...
                logging.exception(
                    "A fatal error has occurred and FADR is exiting.  "
                    "Couldn't create filesystem. Possible permission error")
                notify(job, NOTIFY_TITLE,
                       f"FADR encountered a fatal error processing {job.title}."
            ...
            notify(job, NOTIFY_TITLE, f"FADR Detected a duplicate disc. For {job.title}. "
