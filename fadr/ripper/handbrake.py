...
from fadr.ui import app, db  # noqa E402
...
def handbrake_main_feature(srcpath, basepath, logfile, job):
    ...
    utils.sleep_check_process("HandBrakeCLI", int(cfg.fadr_config["MAX_CONCURRENT_TRANSCODES"]))
    logging.debug("Setting job status to 'transcoding'")
    utils.database_updater({'status': "transcoding"}, job)
    filename = os.path.join(basepath, job.title + "." + cfg.fadr_config["DEST_EXT"])
    ...
    if track is None:
        msg = "No main feature found by Handbrake. Turn main_feature to false in fadr.yml and try again."
        logging.error(msg)
...
def handbrake_all(srcpath, basepath, logfile, job):
    ...
    for track in job.tracks:
        ...
        if int(track.track_number) > job.no_of_titles:
            continue
        if track.length < int(cfg.fadr_config["MINLENGTH"]):
            # too short
            logging.info(f"Track #{track.track_number} of {job.no_of_titles}. "
                         f"Length ({track.length}) is less than minimum length ({cfg.fadr_config['MINLENGTH']}). "
                         f"Skipping...")
        elif track.length > int(cfg.fadr_config["MAXLENGTH"]):
            # too long
            logging.info(f"Track #{track.track_number} of {job.no_of_titles}. "
                         f"Length ({track.length}) is greater than maximum length ({cfg.fadr_config['MAXLENGTH']}). "
                         f"Skipping...")
        else:
            # just right
            ...
            filename = f"title_{track.track_number}.{cfg.fadr_config['DEST_EXT']}"
            filepathname = os.path.join(basepath, filename)
...
def handbrake_mkv(srcpath, basepath, logfile, job):
    ...
    for files in os.listdir(srcpath):
        ...
        for track in job_current_track:
            ...
            track.filename = destfile + "." + cfg.fadr_config["DEST_EXT"]
            logging.debug("UPDATED filename: " + track.filename)
            db.session.commit()
        filename = os.path.join(basepath, destfile + "." + cfg.fadr_config["DEST_EXT"])
        filepathname = os.path.join(basepath, filename)
...
def get_track_info(srcpath, job):
    ...
    cmd = f'{cfg.fadr_config["HANDBRAKE_LOCAL"]} -i {shlex.quote(srcpath)} -t 0 --scan'
    logging.debug(f"Sending command: {cmd}")
