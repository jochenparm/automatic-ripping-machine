...
from fadr.ripper import utils as u
...
def main(disc):
    ...
    if cfg.fadr_config['GET_AUDIO_TITLE'] == 'musicbrainz':
...
def music_brainz(discid, job):
    ...
    """
    mb.set_useragent("fadr", "v2_devel")
    try:
...
def get_title(discid, job):
    ...
    Notes: dont try to use logging here -  doing so will break the fadr setup_logging() function
    """
    mb.set_useragent("fadr", "v2_devel")
    try:
...
if __name__ == "__main__":
    # this will break our logging if it ever triggers for fadr
    disc = Disc("/dev/cdrom")
