...
from fadr.ripper import utils  # noqa: E402
from fadr.ripper.utils import notify
from fadr.ui import db  # noqa: F401, E402
...
def prep_mkv(logfile):
    ...
    try:
        ...
        update_cmd = "/bin/bash /opt/fadr/scripts/update_key.sh"

        # if MAKEMKV_PERMA_KEY is populated
        if cfg.fadr_config['MAKEMKV_PERMA_KEY'] is not None and cfg.fadr_config['MAKEMKV_PERMA_KEY'] != "":
            logging.debug("MAKEMKV_PERMA_KEY populated, using that...")
            # add MAKEMKV_PERMA_KEY as an argument to the command
            update_cmd = f"{update_cmd} {cfg.fadr_config['MAKEMKV_PERMA_KEY']}"
...
def manual_wait(job) -> bool:
    ...
    message = f"FADR has taken it's hands off the wheels. You have {wait_time} minutes to set the job."
    notify(job, title, message)
    ...
    for i in range(wait_time, 0, -1):
        ...
        if job.manual_start:
            ...
            if i % 5 == 0 and i != wait_time:
                body = f"Don't forget me, I need your help to continue doing FADR things!. You have {i} minutes."
                notify(job, title, body)

            if i == 1:
                body = "FADR is about to cancel this job!!! You have less than 1 minute left!"
                notify(job, title, body)
