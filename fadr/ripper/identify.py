...
from fadr.ripper import utils
# flake8: noqa: W605
from fadr.ui import db
from fadr.ui import utils as ui_utils
...
def identify(job):
    ...
    if job.disctype in ["dvd", "bluray"]:
        ...
        if cfg.fadr_config["GET_VIDEO_TITLE"]:
            res = False
            if job.disctype == "dvd":
...
def identify_dvd(job):
    ...
    try:
        ...
        fadr_api_json = json.loads(dvd_info_xml)
        logging.debug(f"dvd xml - {fadr_api_json}")
        logging.debug(f"results = {fadr_api_json['results']}")
        if fadr_api_json['success']:
            logging.info("Found crc64 id from online API")
            logging.info(f"title is {fadr_api_json['results']['0']['title']}")
            args = {
                'title': fadr_api_json['results']['0']['title'],
                'title_auto': fadr_api_json['results']['0']['title'],
                'year': fadr_api_json['results']['0']['year'],
                'year_auto': fadr_api_json['results']['0']['year'],
                'imdb_id': fadr_api_json['results']['0']['imdb_id'],
                'imdb_id_auto': fadr_api_json['results']['0']['imdb_id'],
                'video_type': fadr_api_json['results']['0']['video_type'],
                'video_type_auto': fadr_api_json['results']['0']['video_type'],
                'poster_url': fadr_api_json['results']['0']['poster_img'],
                'poster_url_auto': fadr_api_json['results']['0']['poster_img'],
                'hasnicetitle': True
            }
    ...
    # Do we really need metaselector if we have got from FADR online db?
...
def metadata_selector(job, title=None, year=None):
    ...
    if cfg.fadr_config['METADATA_PROVIDER'].lower() == "tmdb":
        ...
        logging.debug(cfg.fadr_config['METADATA_PROVIDER'])
