...
from fadr.ripper import handbrake, makemkv, utils  # noqa E402
...
def rip_visual_media(have_dupes, job, logfile, protection):
    ...
    if use_make_mkv:
        ...
        try:
            ...
            logging.error(f"MakeMKV did not complete successfully.  Exiting FADR! "
                          f"Error: {mkv_error}")
            raise ValueError from mkv_error

        if makemkv_out_path is None:
            logging.error("MakeMKV did not complete successfully.  Exiting FADR!")
            job.status = "fail"
            db.session.commit()
            sys.exit()
    ...
    # Scan Emby if fadr.yaml requires it
    utils.scan_emby()
    # Set permissions if fadr.yaml requires it
    utils.set_permissions(final_directory)
    # If set in the fadr.yaml remove the raw files
    utils.delete_raw_files([hb_in_path, hb_out_path, makemkv_out_path])
    # report errors if any
    notify_exit(job)
    logging.info("************* FADR processing complete *************")
...
def notify_exit(job):
    """
    Notify post transcoding - FADR finished\n
    Includes any errors
    :param job: current job
    :return: None
    """
