...
/bin/su -l -c "echo /opt/fadr/venv/bin/python3 /opt/fadr/fadr/ripper/main.py -d ${DEVNAME} ${PROTECTION} | at now" -s /bin/bash ${USER}
...
if ! pgrep -f "runui.py" > /dev/null; then
	...
	/bin/su -l -c "/opt/fadr/venv/bin/python3 /opt/fadr/fadr/runui.py  " -s /bin/bash ${USER}
fi
